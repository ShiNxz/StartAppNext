...
const Link = ({ title, icon }) =>
	<a href={`#${title}`} className='rounded-3xl p-4 mb-4 bg-slate-100 flex flex-col justify-center items-center text-gray-600 hover:bg-slate-200 hover:text-gray-800'>
...
const Links = ({ blocks }) => <div className='w-[17%] sticky top-0 h-fit -mt-8'>
	<div className='rounded-3xl shadow-low bg-white p-7 mt-16'>
		{
			blocks?.length > 0 ? blocks.map(b => <Link title={b.title} key={b.title} icon='' />) : (<>none</>)
		}
	</div>
</div>
...
const Profile = ({ params }) => {
	...
	return (
		...
		!data ? (
			<></>
		) : !data?.name ? data.userId ? ( // name missing => user exists
		...
		<ProfileContext.Provider value={{ data, mutate }} >
			...
			<Header name={data.name} title={data.title} avatar={data.avatar} />
			<div className='container mx-auto flex flex-row content-between'>
				<Links blocks={data.blocks} />
				<Blocks>
				{
					data.blocks?.length > 0 ? data.blocks.map(b => <Block key={b.title} info={b} />) : <Block key='none' info={'sdfsd'} />
				}
				</Blocks>
			</div>
		</ProfileContext.Provider>
